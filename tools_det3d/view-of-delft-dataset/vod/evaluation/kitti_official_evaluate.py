...
def clean_data(gt_anno, dt_anno, current_class, difficulty, roi_clean=False, not_roi=False):  # per frame
    ...
    for i in range(num_gt):  # per frame
        ...
        if not_roi:
            x, y, z = gt_anno['location'][i]
            if not (x < left or x > right or z > max_distance):
                ignore = True
...
def _prepare_data(gt_annotations, dt_annotations, current_class, difficulty, custom_method=0):
    ...
    for i in range(len(gt_annotations)):
        ...
        if custom_method == 4:  # results in the not ROI
            rets = clean_data(gt_annotations[i], dt_annotations[i], current_class, difficulty, not_roi=True)
...
def eval_class(gt_annotations,
    ...
    confusion_matrix = np.zeros([num_class, num_difficulty, num_min_overlap, 2, 2], dtype=np.int64)
    gtnums_statistic = np.zeros([num_class, num_difficulty], dtype=np.int64)
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficulties):
            ...
            gtnums_statistic[m, l] = total_num_valid_gt
    ...
    ret_dict = {
        ...
        "gtnums_statistic": gtnums_statistic
    }
    # for l, difficulty in enumerate(difficulties):
    #     for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
    l, k = 0, 0
    tp = ret_dict["confusion_matrix"][:, l, k, :, :][:, 0, 0]
    fp = ret_dict["confusion_matrix"][:, l, k, :, :][:, 0, 1]
    fn = ret_dict["confusion_matrix"][:, l, k, :, :][:, 1, 0]
    # print('difficulty:%d, min_overlap:%d'%(l, k))
    print('car: tp:%4d, fp:%4d, fn:%4d'%(tp[0], fp[0], fn[0]))
    print('ped: tp:%4d, fp:%4d, fn:%4d'%(tp[1], fp[1], fn[1]))
    print('cyc: tp:%4d, fp:%4d, fn:%4d'%(tp[2], fp[2], fn[2]))
    return ret_dict
