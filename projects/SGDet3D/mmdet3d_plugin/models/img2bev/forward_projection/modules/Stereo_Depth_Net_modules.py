...
class DepthAggregation(nn.Module):
    def __init__(self, 
        ...
        self.mapping_mono = nn.Conv2d(input_dims, embed_dims, kernel_size=1, stride=1, padding=0)
        self.mapping_stereo = nn.Conv2d(input_dims, embed_dims, kernel_size=1, stride=1, padding=0)
        self.stem = nn.Conv3d(2, embed_dims, kernel_size=3, stride=1, padding=1)
        self.mono_stereo_attention = Attention(embed_dims, kernel_size=5)
        self.stereo_mono_attention = Attention(embed_dims, kernel_size=5)
        ...
        self.mapping_out = nn.Conv2d(embed_dims, input_dims, kernel_size=1, stride=1, padding=0)
        
    def forward(self, depth_stereo, depth_mono):
        
        depth_stereo = self.mapping_stereo(depth_stereo)
        depth_mono = self.mapping_mono(depth_mono)
        ...
        depth_prob = self.mapping_out(depth_prob)
