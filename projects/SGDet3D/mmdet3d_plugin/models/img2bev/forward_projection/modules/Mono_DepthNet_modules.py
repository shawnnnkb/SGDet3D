...
class DepthNet2(nn.Module):
    def __init__(self, in_channels, mid_channels, context_channels,
                 depth_channels, cam_channels=27):
        super(DepthNet2, self).__init__()
        
        # input from in_channels to mid_channels
        self.reduce_conv = nn.Sequential(
        ...
        # camera aware mlp input for mlp and SE layer
        self.bn = nn.BatchNorm1d(cam_channels)
        self.depth_mlp = Mlp(cam_channels, mid_channels, mid_channels)
        self.depth_se = SELayer(mid_channels)  # NOTE: add camera-aware
        self.context_mlp = Mlp(cam_channels, mid_channels, mid_channels)
        ...
        # depth: from mid_channels to depth_channels
        # context: from mid_channels to context_channels
        self.context_conv = nn.Conv2d(mid_channels, context_channels, kernel_size=1,  stride=1, padding=0)
        if mid_channels != depth_channels:
            downsample = nn.Conv2d(mid_channels, depth_channels, 3, 1, 1)
        else:
            downsample = None
        self.depth_conv = nn.Sequential(
    ...
    def forward(self, x, mlp_input):
        ...
        # depth branch
        depth_se = self.depth_mlp(mlp_input)[..., None, None]
        depth_tmp = self.depth_se(x, depth_se)
        depth = self.depth_conv(depth_tmp)
