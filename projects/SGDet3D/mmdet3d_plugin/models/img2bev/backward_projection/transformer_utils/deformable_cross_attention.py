...
from mmcv.cnn.bricks.transformer import build_attention, ATTENTION
import math
...
class SpatialCrossAttention_DFA3D(SpatialCrossAttention):
    ...
    def forward(self,
        ...
        D = reference_points_cam.size(3)
        indexes = []
        # here, B=1, thus using 'mask_per_img[0]'
        # TODO: support batch size > 1 when not using multi_apply()
        for i, mask_per_img in enumerate(bev_mask): # [num_cam, B, num_query, D]
            index_query_per_img = mask_per_img[0].sum(-1).nonzero().squeeze(-1)
            indexes.append(index_query_per_img)
        max_len = max([len(each) for each in indexes])
        # each camera only interacts with its corresponding BEV queries. This step can  greatly save GPU memory.
